...
## 11. 'quad'/'six' also left grid placement to auto-flow, same as 'five' before item 8 (Low, defense-in-depth) — Fixed

Follow-up to item 8, prompted by a direct question about whether the 'six' layout (3 cols × 2 rows, same `style:'grid'` as 'quad') had the analogous bug. It didn't in practice — 'quad'/'six' use uniform 1×1 cells with no `span`, and CSS Grid's row-major auto-placement for same-size items is unambiguous per spec (verified empirically: all 4/6 panels place and hit-test correctly in headless Chromium, same as before this change). But leaving them on implicit auto-placement while 'five' was pinned explicit was an inconsistency in the same risk area (the embedded JUCE WebView noted in CLAUDE.md). Fixed by giving `createPanel()`'s plain `style:'grid'` branch the same explicit `gridRow`/`gridColumn` (computed from `index` and `cfg.cols`) that 'five' already has, so no grid-style layout depends on auto-placement.
